...
class TimeTableRamadanMonthHistoryController extends Controller
{
    public function index()
    {                      
        $pageNameAr = 'سجل جدول الحصص لشهر رمضان';
        $pageNameEn = 'time_table_ramadan_month_histories';
        return view('back.time_table_ramadan_month_history.index' , compact('pageNameAr' , 'pageNameEn'));
    }

    public function lastOrderNumber($id)
    {                
        $lastOrderNumber = TimeTableRamadanMonthHistory::where('category', $id)->orderBy('order', 'desc')->first();
        if(!$lastOrderNumber){
            $lastOrderNumber = 1;
        }else{
    ...
    public function store(Request $request)
    {
        if (request()->ajax()){
            $this->validate($request , [
                'category' => 'required',
                'name_ar' => 'required|unique:time_table_ramadan_month_histories,name_ar',
            ],[
            ...
            TimeTableRamadanMonthHistory::create($request->all());
        }
    }
    ...
    {
        if(request()->ajax()){
            $find = TimeTableRamadanMonthHistory::where('id', $id)->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        // dd($request->all());
            $this->validate($request , [
                'category' => 'required',
                'name_ar' => 'required|unique:time_table_ramadan_month_histories,name_ar,'.$id,
            ],[
                'required' => ':attribute مطلوب.',
                'integer' => ':attribute غير صحيح.',
    ...
    {
        $all = TimeTableRamadanMonthHistory::orderBy('id', 'desc')
                                ->leftJoin('tbl_groups', 'tbl_groups.ID', 'time_table_ramadan_month_histories.group_id_time')
                                ->leftJoin('tbl_rooms', 'tbl_rooms.RoomID', 'time_table_ramadan_month_histories.room_id_time')
                                ->leftJoin('users', 'users.id', 'time_table_ramadan_month_histories.user_id')
                                ->select(
                                        "time_table_ramadan_month_histories.*", 
