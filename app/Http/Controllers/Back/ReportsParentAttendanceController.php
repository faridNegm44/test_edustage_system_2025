...
class ReportsParentAttendanceController extends Controller
{
    public function index()
    {         
        //if((userPermissions()->clients_report_view)){
            $pageNameAr = '📋 كشف تفقد حضور وغياب الطلاب 👨‍👩‍👦';        
            $parents = DB::table('tbl_parents')->select('ID', 'TheName0')->orderBy('TheName0', 'asc')->get();   
            
            return view('back.reports.parent_attendance.index' , compact('pageNameAr', 'parents'));
    ...
    {                   
        ...
        if(count($results) == 0){
            ...
            return view('back.reports.parent_attendance.pdf' , compact('pageNameAr', 'results', 'from', 'to'));
        }
    }
}