...
use App\Models\Back\TimeTableRamadanMonth;
use App\Models\Back\TimeTableRamadanMonthHistory;
use App\Models\Back\Times;
use Illuminate\Support\Facades\Cache;

class TimeTableRamadanMonthController extends Controller
{
    public function daysClasses($dayAr, $userNum){
        if(auth()->user()->user_status == 1 || auth()->user()->user_status == 2){   // start check if auth()->user()->user_status == 1 || 2  => admin or employ
            return DB::table('time_tables_ramadan_month')
                    ->where('day', $dayAr)
                    ->where('user', $userNum)
                    ...
                    ->orderBy('time_tables_ramadan_month.room_id', 'ASC')
                    ->get();
    ...
    {
        $pageNameAr = 'جدول الحصص للمدرسين لشهر رمضان';
        $pageNameEn = 'time_table_ramadan_month';
        ...
        return view('back.time_table_ramadan_month.index' , compact('pageNameAr' , 'pageNameEn', 'groups', 'rooms', 'times', 'satClassesUserOne', 'satClassesUserTwo', 'sunClassesUserOne', 'sunClassesUserTwo', 'monClassesUserOne', 'monClassesUserTwo', 'tueClassesUserOne', 'tueClassesUserTwo', 'wedClassesUserOne', 'wedClassesUserTwo', 'thuClassesUserOne', 'thuClassesUserTwo', 'friClassesUserOne', 'friClassesUserTwo'));
    }

    public function get_available_times_to_add_form(Request $request)
    {
        $timesToTimeTable = DB::table('time_tables_ramadan_month')
            ...
            if(auth()->user()->user_status == 4){  // start check لو المستخدم الي مسجل مدرس
                ...
                if($isEqual){
                    DB::transaction(function(){
                        ...
                        $lastId = $this->getLastId('time_tables_ramadan_month', 'group_to_colspan');
                        ...
                        TimeTableRamadanMonth::insert($data);

                        TimeTableRamadanMonthHistory::create([
        ...
        if (request()->ajax()){
            ...
            if(!request('times')){
                ...
                DB::transaction(function() use ($group_to_colspan) {
                    $findHistory = TimeTableRamadanMonthHistory::where('relation_id', $group_to_colspan)->first();

                    TimeTableRamadanMonth::where('group_to_colspan', $group_to_colspan)->update([
                        'class_type' => request('class_type'),
                        'notes' => request('notes'),
    ...
    {
        if (request()->ajax()){
            $findTimesTimeTable = TimeTableRamadanMonth::where('group_id', $group_id)
            ...
            $plukTimes = TimeTableRamadanMonth::where('group_id', $group_id)->where('group_to_colspan', $group_to_colspan)->pluck('times')->toArray();
    ...
    public function remove_recorded_times(Request $request){
        ...
        $getInfoFirst = TimeTableRamadanMonth::where('id', $values[0])->first();
        $getInfoAllCount = TimeTableRamadanMonth::where('group_to_colspan', $getInfoFirst->group_to_colspan)->count();
        ...
        DB::transaction(function() use ($values, $texts, $getInfoFirst, $getInfoAllCount, $notes){
            ...
            TimeTableRamadanMonth::whereIn('id', $values)->delete();

            $firstRow = TimeTableRamadanMonth::where('group_to_colspan', $getInfoFirst->group_to_colspan)->orderBy('id', 'asc')->first();
            $latestRow = TimeTableRamadanMonth::where('group_to_colspan', $getInfoFirst->group_to_colspan)->orderBy('id', 'desc')->first();

            if($firstRow && $latestRow){
                TimeTableRamadanMonth::where('group_to_colspan', $getInfoFirst->group_to_colspan)
                        ->update([
