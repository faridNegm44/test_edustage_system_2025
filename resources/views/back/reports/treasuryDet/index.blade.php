                        ...
                        <div id="div_datatable">
                            <table class="table table-hover dt-responsive w-100 table-striped table-bordered text-center">
                                ...
                                <tr style="font-weight: bold;">
                                    ...
                                    <th rowspan="2">نوع المعاملة</th>
                                    <th colspan="3">حركة المقبوضات</th>
                                    <th colspan="3">حركة المدفوعات</th>
            ...
            // start change branch
                $(document).on('change' , '#selectBranch' , function () {
                    ...
                    if(selectedValue != null){
                        $.ajax({
                            url: `{{ url('reports/treasuryDet/getTreasuries') }}/${selectedValue}`,
                            type: 'GET',
                            dataType: 'json',
                            beforeSend:function () {
                                $("#allClienstDet").empty();
                            ...
                            success: function(res) {
                                if(res.length){
                                    ...
                                    alertify.success("تم جلب الخزائن المتعلقين بهذا الفرع");

                                    $("#allClienstDet").append(`<option value="" selected disabled>-- اختر خزينة --</option>`);
            ...
            // start search
                $("#searchArea #search").click(function(){
                    ...
                    if(!branchVal || !allClienstDet){
                        ...
                        formData.append("allTreasuryDet", allClienstDet);
                        formData.append("from", from);
                        formData.append("to", to);

                        $.ajax({
                            url: `{{ url('reports/treasuryDet/view') }}`,
                            type: 'POST',
                            contentType: false,
                            cache: false,
                            processData: false,
